# Internal compatibility code adapted from the development version of
# `mixmgfa` by Kim De Roover and colleagues.
#
# These functions are included to make `MixMix_Step1()` reproducible while the
# public GitHub version of `mixmgfa` does not yet include the updated
# `invar_loadings` argument and related scale/rotate functionality.
#
# These functions are not exported and are not presented as original
# `mix2mgsem` contributions. Once the upstream `mixmgfa` implementation is
# publicly updated, this compatibility code should be replaced by a direct
# dependency on `mixmgfa`.

#' Scale and rotate mixmgfa output (with partial invariant loadings)
#' @noRd
.scale_rotate_mixmgfa <- function(OutputObject,N_gs,cluster.spec,nsclust=c(),design=0,rescale=0,markers=0,rotation=0,targetT=0,targetW=0){
